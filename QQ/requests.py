...
class CheckRequest(WebQQRequest):
    ...
    def callback(self, resp, data):
        ...
        r, vcode, uin, w = eval("self.hub."+data.strip().rstrip(";"))
...
class BeforeLoginRequest(WebQQRequest):
    ...
    def callback(self, resp, data):
        ...
        #logger.info('resp:{0}'.format(resp.headers))
        match = re.search(r'(?<=skey=).*?(?=;)', str(resp.headers))
        if match:
            skey = match.group()
            logger.info('skey:{0}'.format(skey))
            self.hub.skey = skey
        else:
            logger.warn('获取skey失败！')
...
class GroupListRequest(WebQQRequest):
    ...
    def callback(self, resp, data):
        ...
        for i, group in enumerate(group_list):
            ...
            self.hub.load_next_request(GroupMembersRequest(gcode, self.guanggao, i == 0))
...
class GroupMembersRequest(WebQQRequest):
    ...
    def init(self, gcode, guanggao,poll = False):
        self._poll = poll
        self._gcode = gcode
        self.guanggao = guanggao
        self.to_the_uin = None
        self.gid = None
    ...
    def send(self):
        time.sleep(1)
        logger.info('发送临时消息给：{0}'.format(self.to_the_uin))
        self.hub.send_sess_msg(self.gid, self.to_the_uin, self.guanggao)
        time.sleep(1)
    ...
    def callback(self, resp, data):
        ...
        self.gid = data.get("result", {}).get("ginfo", []).get('gid')
        self.hub.group_members_info[self._gcode] = {}
        for i,m in enumerate(members):
            uin = m.get("uin")
            #self.hub.send_sess_msg(gid, uin, self.guanggao)
            logger.info('i:{0}, uin:{1}'.format(i, uin))
            self.to_the_uin = uin
            tornado.ioloop.IOLoop.instance().add_timeout(time.time()+(i+1)*3, callback=self.send)
...
class PollMessageRequest(WebQQRequest):
    ...
    def callback(self, resp, data):
        try:
            ...
            logger.info(u"消息获取异常: {0}".format(e), exc_info = True)
        finally:
...
class HandleGroupAdd(WebQQRequest):
    """1.QQ号是否在黑名单中"""

    url = 'http://s.web2.qq.com/api/get_friend_uin2'

    def init(self, group_uin, req_uin, gcode, msg):
        self.group_uin = group_uin
        self.req_uin = req_uin
        self.gcode = gcode
        self.msg = msg

        self.params = (('tuin', req_uin), ('verifysession',''),
                    ('type',1), ('code', ''),
                    ('vfwebqq',self.hub.vfwebqq), ('t', int(time.time())))
        self.headers.update(Referer = const.S_REFERER)

    def callback(self, resp, data):
        if data.get('retcode') == 0:
            qqnum = data.get('result').get('account')
            if self.hub.db_query('guanggao.db', qqnum):
                tishi = ''
                logger.info('QQ %s is in heimingdan.' % qqnum)
                return self.hub.load_next_request(OperateGroupAdd(self.group_uin, self.req_uin, 3, qqnum, self.gcode, self.msg, tishi))
            else:
                logger.info('Not in heimingdan.')
                return self.hub.load_next_request(GroupMemberNumber(self.group_uin, self.req_uin, qqnum, self.gcode, self.msg))

class GroupMemberNumber(WebQQRequest):
    """2.获取群是否满员"""

    url = 'http://s.web2.qq.com/api/get_group_public_info2'

    def init(self, group_uin, req_uin, qqnum, gcode, msg):
        self.group_uin = group_uin
        self.req_uin = req_uin
        self.qqnum = qqnum
        self.gcode = gcode
        ...
        self.params = (('gcode', gcode), ('vfwebqq', self.hub.vfwebqq), ('t', int(time.time())))

    def callback(self, resp, data):
        if data.get('retcode') == 0:
            member_cnt = data.get('result').get('ginfo').get('member_cnt')
            maxmember = data.get('result').get('ginfo').get('maxmember')

            if member_cnt == maxmember:
                tishi = '群成员已满，请联系群主。'
                logger.info('tishi:{0}, member_cnt:{1}'.format(tishi, member_cnt))
                return self.hub.load_next_request(OperateGroupAdd(self.group_uin, self.req_uin, 3, self.qqnum, self.gcode, self.msg, tishi))
            else:
                logger.info('member_cnt:{0}'.format(member_cnt))
                tishi = ''
                return self.hub.load_next_request(OperateGroupAdd(self.group_uin, self.req_uin, 2, self.qqnum, self.gcode, self.msg, tishi))

class SingleLongNick(WebQQRequest):
    """3.获取申请入群者的个性签名"""
    ...
    def init(self, group_uin, req_uin, qqnum, gcode, msg):
        ...
        self.params = (('tuin', req_uin), ('vfwebqq', self.hub.vfwebqq), ('t', int(time.time())))

    def callback(self, resp, data):
        logger.info('SingleLongNick data:{0}'.format(data))
        if data.get('retcode') == 0:
            lnick = data.get('result')[0].get('lnick')
            logger.info('lnick:{0}'.format(lnick))
            if '出售' in lnick:
                tishi = ''
                return self.hub.load_next_request(OperateGroupAdd(self.group_uin, self.req_uin, 3, self.qqnum, self.gcode, self.msg, tishi))
            else:
                return self.hub.load_next_request(QQLevel(self.group_uin, self.req_uin, self.qqnum, self.gcode, self.msg))
        else:
            return self.hub.load_next_request(QQLevel(self.group_uin, self.req_uin, self.qqnum, self.gcode, self.msg))


class QQLevel(WebQQRequest):
    """4.获取申请入群者的Q龄"""

    url = 'http://s.web2.qq.com/api/get_qq_level2'
    ...
    def callback(self, resp, data):
        logger.info('QQlevel data:{0}'.format(data))
        if data.get('retcode') == 0:
            days = data.get('result').get('days')
            year = days/365 + 1
            if year <= 2:
                optype = 3
                tishi = 'Q龄太小，请用大号加群。'
                ...
                optype = 3
                tishi = '避免广告骚扰，Q龄太高不予入群；如有疑问，请联系管理员。'
            else:
                tishi = ''
                optype = 2
            logger.info('QQlevel:{0}days, optype:{1}, tishi:{2}'.format(days, optype, tishi))
            return self.hub.load_next_request(OperateGroupAdd(self.group_uin, self.req_uin, optype, self.qqnum, self.gcode, self.msg, tishi))
        else:
            optype = 3
            tishi = 'Q龄太小，请用大号加群。'
            return self.hub.load_next_request(OperateGroupAdd(self.group_uin, self.req_uin, optype, self.qqnum, self.gcode, self.msg, tishi))

class OperateGroupAdd(WebQQRequest):
    """5.处理入群请求"""

    url = "http://d.web2.qq.com/channel/op_group_join_req"
    
    def init(self, group_uin, req_uin, optype, qqnum, gcode, msg, tishi):
        self.qqnum = qqnum
        self.gcode = gcode
        self.msg = msg
        self.optype = optype
        ...
        self.params = (('group_uin', group_uin), ('req_uin', req_uin), 
                ('msg', tishi), ('op_type', optype), ('clientid', self.hub.clientid), 
                ('psessionid', self.hub.psessionid), ('t', time.time()))

    def callback(self, resp, data):
        if self.optype == 2:
            if data.get('retcode') == 0:
                new_msg = "大家热烈欢迎新研友：【{0}】\nTa填写的入群信息是：【{1}】.".format(self.qqnum, self.msg)
                #new_msg = self.msg
                if self.hub.last_group_msg != new_msg:
                    self.hub.send_group_msg(self.gcode, new_msg)
                    logger.info(u"Add {0} Successful!".format(self.qqnum))
                self.hub.last_group_msg = new_msg
            else:
                logger.info(u"Add {0} Failed!".format(self.qqnum))
        else:
            if data.get('retcode') == 0:
                logger.info('Deny {0} Successful!'.format(self.qqnum))

class ZhiLiao(WebQQRequest):
    url = 'http://app.simsimi.com/app/aicr/request.p'
    method = WebQQRequest.METHOD_POST

    def init(self, from_uin, content):
        self.uin = from_uin
        self.params = [('req', content), ('uid', 54319759), ('lc', 'zh'), ('tz', 'Asia/Chongqing'), ('ft',0), ('os', 'a'), ('av','5.3.2')]

    def callback(self, resp, data):
        try:
            content=data.get('sentence_resp')
            if 'Sim'in content:
                content='Bu dong ni zai shuo sha!'
            if 'chat' in content:
                content='...'
        except Exception:
            content='...'
        self.hub.send_buddy_msg(self.uin, content)
        #logger.info("ZhiLiao res:{0}, data:{1}, content:{2}".format(resp, data, content))

class ChaIP(WebQQRequest):
    url = 'http://wap.ip138.com/ip_search.asp'

    def init(self, from_uin, ip):
    ...
    def callback(self, resp, data):
        try:
            p = re.compile(r'(?<=<b>).*?(?=</b>)')
            msg = p.findall(data)[0]
        except Exception,e:
            msg = str(e)
        self.hub.send_buddy_msg(self.from_uin, msg)

class ChaPhone(WebQQRequest):
    url = 'http://wap.ip138.com/sim_search.asp'

    def init(self, from_uin, phone):
        self.from_uin = from_uin
        self.phone = phone
        self.params = (('mobile', phone),)

    def callback(self, resp, data):
        p1 = re.compile(r'(?<=归属地：).*?(?=<br/>)')
        p2 = re.compile(r'(?<=卡类型：).*?(?=<br/>)')

        try:
            msg1 = p1.findall(data)[0].replace('<div>', '').replace('</br>', '')
            msg2 = p2.findall(data)[0].replace('<div>', '').replace('</br>', '')
            msg = '归属地：%s  卡类型：%s' % (msg1, msg2)
        except Exception,e:
            msg = 'Not found!'
        self.hub.send_buddy_msg(self.from_uin, msg)

class DelGroupMember_QQ(WebQQRequest):
    """1.获取广告成员QQ号"""
    url = 'http://s.web2.qq.com/api/get_friend_uin2'

    def init(self, send_uin, content, group_code):
        self.send_uin = send_uin
        self.content = content
        self.group_code = group_code

        self.params = (('tuin', send_uin), ('verifysession', ''), ('type', 1), ('code',''), ('vfwebqq', self.hub.vfwebqq), ('t', int(time.time())))

    def callback(self, resp, data):
        if data.get('retcode') == 0:
            qqnum = data.get('result').get('account')
            return self.hub.load_next_request(DelGroupMember_Qun(qqnum, self.content, self.group_code))
        else:
            logger.info('Del {0} Failed!'.format(self.send_uin))

class DelGroupMember_Qun(WebQQRequest):
    """2.获取群号"""
    url = 'http://s.web2.qq.com/api/get_friend_uin2'

    def init(self, qqnum, content, group_code):
        self.qqnum = qqnum
        self.content = content
        self.group_code = group_code

        self.params = (('tuin', group_code), ('verifysession', ''), ('type', 4), ('code', ''), ('vfwebqq', self.hub.vfwebqq), ('t', int(time.time())))

    def callback(self, resp, data):
        if data.get('retcode') == 0:
            gcode = data.get('result').get('account')
            return self.hub.load_next_request(DelGroupMember(self.qqnum, self.content, gcode, self.group_code))

class DelGroupMember(WebQQRequest):
    """3.删除群成员。"""
    url = 'http://qinfo.clt.qq.com/cgi-bin/qun_info/delete_group_member'
    method = WebQQRequest.METHOD_POST

    def init(self, qqnum, content, gcode, group_code):
        self.qqnum = qqnum
        self.content = content
        self.gcode = gcode
        self.group_code = group_code

        bkn = self.get_gtk(self.hub.skey)
        
        self.params = [('gc', gcode), ('ul', qqnum), ('bkn', bkn)]
    ...
    def callback(self, resp, data):
        ...
        if data.get('ec') == 0:
            self.logheimingdan('/root/QQ/guanggao.db', self.qqnum, self.content, self.gcode)
            logger.info('踢出{0}群的{1}成功，已记录到黑名单。'.format(self.gcode, self.qqnum))
            self.hub.send_group_msg(self.group_code, '{0}打广告，已T。'.format(self.qqnum))
        else:
            logger.info('踢出{0}群的{1}失败，data：{2}'.format(self.gcode, self.qqnum, data))


    def logheimingdan(self, dbname, qqnum, content, gcode):
        logger.info('DB:%s' % dbname)
        conn = sqlite3.connect(dbname)
        cur = conn.cursor()
        
        Ttime = int(time.time())
        cur.execute("insert into heimingdan values(%s,'%s',%s,%s)" % (qqnum, content, gcode, Ttime))
...
def system_message_handler(func):
    ...
    def args_func(self, message):
        value = message.get('value')
        return (value.get("type"), value.get("from_uin"), value.get('account'), message)
    return _register_message_handler(func, args_func, "system_message")

def system_group_add(func):
    """Decorate group add"""
    def args_func(self, message):
        value = message.get('value')
        return (value.get('type'), value.get('from_uin'), value.get('request_uin'), message)
    return _register_message_handler(func, args_func, "sys_g_msg")
