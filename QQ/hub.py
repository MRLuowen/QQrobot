...
from .asynchttpclient import UploadForm as Form
...
from .requests import HandleGroupAdd
from .requests import ZhiLiao
from .requests import ChaIP
from .requests import ChaPhone
from .requests import DelGroupMember_QQ
from .requests import GroupListRequest
...
class RequestHub(object):
    ...
    def __init__(self, qid, pwd, client = None, debug = False):
        ...
        # 检查是否验证码的回调
        self.ptui_checkVC = lambda r, v, u, w: (r, v, u, w)
    ...
    def _hash(self):
        """  获取好友列表时的Hash """
        my_hash = _hash.webqq_hash_one(self.qid, self.ptwebqq)
        if len(my_hash)==16:
            my_hash = _hash.webqq_hash_two(self.qid, self.ptwebqq) 
        #logger.info('my_hash:%s' % my_hash)
        return my_hash
    ...
    def make_msg_content(self, content):
        ...
        """
        self.msg_id += 1
        msg = json.dumps([content, ["font", {"name":"Monospace", "size":10,
    ...
    def operate_group_add(self, group_uin, req_uin, gcode, msg):
        """Operate group add request
        :param group_uin: which group want to add
        :param req_uin: who want to add group
        :param optype: accept or deny
        """
        return self.load_next_request(HandleGroupAdd(group_uin, req_uin, gcode, msg))

    def zhiliao(self, from_uin, content):
        return self.load_next_request(ZhiLiao(from_uin, content))

    def chaip(self, from_uin, ip):
        return self.load_next_request(ChaIP(from_uin, ip))

    def chaphone(self, from_uin, phone):
        return self.load_next_request(ChaPhone(from_uin, phone))

    def db_init(self, dbname):
        conn = sqlite3.connect(dbname)
        cur = conn.cursor()

        cur.execute('create table heimingdan(qq integer primary key,content text,gcode integer,Ttime integer)')

        conn.commit()
        conn.close()

    def db_query(self, dbname, qqnum):
        ...
        sql = 'select qq from heimingdan where qq=%s' % qqnum
    ...
    def del_group_member(self, send_uin, content, gcode):
        self.load_next_request(DelGroupMember_QQ(send_uin, content, gcode))

    def qunfa(self, guanggao):
        self.load_next_request(GroupListRequest(guanggao))
